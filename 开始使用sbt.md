...
##大致翻译一下（水平有限，欢迎指正。）：
        ...
        #开始使用sbt
sbt 只需少量的概念即可提供灵活且强大的 build definitions.其它构建工具能让你在无意中发现它们的细节，就算你从来没有阅读过它们的相关文档， 但是只有为数不多的概念 sbt是个例外。
此入门教程包含了你需要知道创建和维护sbt  build definition 的相关概念。
因此，强烈推荐去阅读此入门教程。
如果你已经迫不及待了，最重要概念的背景可以在.sbt build definition, scopes, 和章节找到。但是我们不保证这是一个好主意去跳过此教程的其它章节。最好是按顺序阅读，因为后面章节的概念会在前面章节有介绍到。
感谢尝试sbt，祝你开心。
